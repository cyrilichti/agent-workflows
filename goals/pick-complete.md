...
- An approved plan caused the item to move to `in progress` before `/work` was
  invoked with the updated official item context.
- A `needs-refinement` result left the parent item unchanged by `/pick`,
  honored the user's refinement choice, and stopped without invoking `/work`.
- Observed provider results were reported without inferring a successful
  mutation.

## Stop Conditions

- Stop successfully after invoking `/work` from an approved plan and a
